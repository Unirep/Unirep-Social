...
describe('Subsidy', function () {
    ...
    it.skip('should create a vote using subsidy', async () => {
        ...
        let toProofIndex
        {
            ...
            await unirepSocialContract
                ...
                .then((t) => t.wait())
            toProofIndex = await unirepContract.getProofIndex(repProof.hash())
        }
        // now create a vote
        const id = new ZkIdentity()
        await unirepSocialContract
    ...
    it.skip('should partially spend using subsidy', async () => {
            ...
            .then((t) => t.wait())
        {
            ...
            const proofIndexes = await Promise.all(
                [startTransitionProof, ...processAttestationProofs].map((p) =>
                    unirepContract.getProofIndex(p.hash())
                )
            )
