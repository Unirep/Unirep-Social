...
const genReputationProof = async (args: any) => {
    ...
    if(args.action == 'post') {
        ...
        if(args.vote_value == undefined) {
            ...
            return
        } else if(args.vote_value > maxReputationBudget || args.vote_value < 1) {
            console.error(`Error: should provide a valid vote value, min: 1, max: ${maxReputationBudget}`)
            return
        }
        proveReputationAmount = args.vote_value
    } else if(args.reputation_nullifier != null) {
        proveReputationAmount = args.reputation_nullifier != null ? args.reputation_nullifier : 0
    } else {
