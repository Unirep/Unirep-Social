...
const UnirepSocial_json_1 = __importDefault(
...
class UnirepSocialContract {
    constructor(unirepSocialAddress, provider) {
        this.unlock = async (eth_privkey) => {
            ...
            if (
                !(await utils_1.checkDeployerProviderConnection(
                    ethSk,
                    this.provider
                ))
            ) {
        ...
        this.getUnirep = async () => {
            ...
            this.unirep = contracts_1.getUnirepContract(
                unirepAddress,
                this.provider
            )
            return this.unirep
        ...
        this.currentEpoch = async () => {
            ...
            return (_a = this.unirep) === null || _a === void 0
                ? void 0
                : _a.currentEpoch()
        }
        this.attesterId = async () => {
            ...
            return (_a = this.unirep) === null || _a === void 0
                ? void 0
                : _a.attesters(this.contract.address)
        }
        ...
        this.publishPost = async (reputationProof, postContent) => {
            ...
            const attestingFee = await this.attestingFee()
            return this.contract.publishPost(postContent, reputationProof, {
                value: attestingFee,
                gasLimit: 1000000,
            })
        }
        ...
        this.getReputationProofIndex = async (reputationProof) => {
            ...
            return (_a = this.unirep) === null || _a === void 0
                ? void 0
                : _a.getProofIndex(reputationProof.hash())
        }
        ...
        this.submitStartTransitionProof = async (startTransitionProof) => {
            ...
            return this.contract.startUserStateTransition(
                ...
                circuits_1.formatProofForVerifierContract(
        ...
        this.getStartTransitionProofIndex = async (startTransitionProof) => {
            ...
            const proofNullifier = contracts_1.computeStartTransitionProofHash(
                startTransitionProof.blindedUserState,
                startTransitionProof.blindedHashChain,
                startTransitionProof.globalStateTreeRoot,
                circuits_1.formatProofForVerifierContract(
            ...
            return (_a = this.unirep) === null || _a === void 0
                ? void 0
                : _a.getProofIndex(proofNullifier)
        }
        this.submitProcessAttestationsProof = async (
            processAttestaitonProof
        ) => {
            if (this.signer != undefined) {
                this.contract = this.contract.connect(this.signer)
            } else {
            ...
            return this.contract.processAttestations(
                ...
                circuits_1.formatProofForVerifierContract(
                    processAttestaitonProof.proof
                )
            )
        }
        this.getProcessAttestationsProofIndex = async (
            processAttestaitonProof
        ) => {
            var _a
            if (this.unirep == undefined) {
            ...
            const proofNullifier =
                contracts_1.computeProcessAttestationsProofHash(
                    processAttestaitonProof.outputBlindedUserState,
                    processAttestaitonProof.outputBlindedHashChain,
                    processAttestaitonProof.inputBlindedUserState,
                    circuits_1.formatProofForVerifierContract(
        ...
        this.submitUserStateTransitionProof = async (
            USTProof,
            proofIndexes,
            nonce
        ) => {
        ...
        this.userStateTransition = async (results) => {
            const txList = []
            const proofIndexes = []
            let tx = await this.submitStartTransitionProof(
                results.startTransitionProof
            )
            txList.push(tx)
            await tx.wait()
            for (let i = 0; i < results.processAttestationProofs.length; i++) {
                tx = await this.submitProcessAttestationsProof(
                    results.processAttestationProofs[i]
                )
                txList.push(tx)
                await tx.wait()
            }
            const proofIndex = await this.getStartTransitionProofIndex(
                results.startTransitionProof
            )
            proofIndexes.push(BigInt(proofIndex))
            for (let i = 0; i < results.processAttestationProofs.length; i++) {
                const proofIndex = await this.getProcessAttestationsProofIndex(
                    results.processAttestationProofs[i]
                )
                proofIndexes.push(BigInt(proofIndex))
            }
            const USTProof = new contracts_1.UserTransitionProof(
                results.finalTransitionProof.publicSignals,
                results.finalTransitionProof.proof
            )
            tx = await this.submitUserStateTransitionProof(
                USTProof,
                proofIndexes
            )
            txList.push(tx)
        ...
        this.airdrop = async (signUpProof) => {
            ...
            const attestingFee = await this.attestingFee()
            return this.contract.airdrop(signUpProof, {
                value: attestingFee,
                gasLimit: 1000000,
            })
        }
        ...
        this.verifyEpochKeyValidity = async (epochKeyProof) => {
            ...
            const isValid = await ((_a = this.unirep) === null || _a === void 0
                ? void 0
                : _a.verifyEpochKeyValidity(epochKeyProof))
            return isValid
        }
        this.verifyReputation = async (reputationProof) => {
            ...
            const isValid = await ((_a = this.unirep) === null || _a === void 0
                ? void 0
                : _a.verifyReputation(reputationProof))
            return isValid
        }
        this.verifyUserSignUp = async (signUpProof) => {
            ...
            const isValid = await ((_a = this.unirep) === null || _a === void 0
                ? void 0
                : _a.verifyUserSignUp(signUpProof))
            return isValid
        }
        ...
        this.contract = new ethers_1.ethers.Contract(
            unirepSocialAddress,
            UnirepSocial_json_1.default.abi,
            this.provider
        )
    }
}
