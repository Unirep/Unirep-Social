...
describe('Integration', function () {
    ...
    before(async () => {
        ...
        unirepSocialId = BigInt(await unirepContract.attesters(unirepSocialContract.address))
    })
    ...
    describe('Second epoch', () => {
        ...
        it('first user generate an epoch key and verify it', async () => {
            ...
            const { publicSignals, proof } = await users[firstUser].genVerifyEpochKeyProof(epochKeyNonce)
        ...
        it('first user publish a post and generate epoch key', async () => {
            ...
            const postFilter = unirepSocialContract.filters.PostSubmitted(currentEpoch, epochKey)
        ...
        it('Second user upvote to first user', async () => {
            ...
            const proveGraffiti = BigInt(0)
            const graffitiPreImage = genRandomSalt()
            const nonceList: BigInt[] = findValidNonce(users[secondUser], repNullifiersAmount, currentEpoch.toNumber(), unirepSocialId)
            ...
            const _reputationProofIndex = await unirepContract.getProofIndex(reputationProof.hash())
            console.log(`Attester attest to epk ${firstUserEpochKey} with proof index ${_reputationProofIndex.toNumber()}`)
        })
        ...
        it('first user leave a comment and generate epoch key', async () => {
            ...
            // compute reputation proof index
            const _reputationProofIndex = await unirepContract.getProofIndex(reputationProof.hash())
            console.log(`Attester attest to epk ${epochKey} with proof index ${_reputationProofIndex.toNumber()}`)
        })

        it('First user request Unirep Social for epoch transition airdrop', async () => {
            const { publicSignals, proof } = await users[firstUser].genUserSignUpProof(unirepSocialId)
            ...
            // compute reputation proof index
            const _reputationProofIndex = await unirepContract.getProofIndex(signUpProof.hash())
            console.log(`Attester attest to epk ${signUpProof.epochKey} with proof index ${_reputationProofIndex.toNumber()}`)
        })
        ...
        it('Attestations gathered from events should match', async () => {
            ...
            for (let epochKey of secondEpochEpochKeys) {
                ...
                let attestations_: IAttestation[] = attestationsByEpochKeyEvent.map((event: any) => event['args']['_attestation'])
    ...
    describe('Third epoch', () => {
        ...
        it('First user prove his reputation', async () => {
            ...
            const { publicSignals, proof } = await users[firstUser].genProveReputationProof(attesterId, epkNonce, minRep, proveGraffiti, graffitiPreImage)
            const reputationProof = new ReputationProof(publicSignals, proof)
            const isValid = await reputationProof.verify()
            expect(isValid, 'Verify reputation proof off-chain failed').to.be.true
